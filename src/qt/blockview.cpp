...
{
    qRegisterMetaType<std::shared_ptr<BubbleGraph>>("std::shared_ptr<BubbleGraph>");
...
void GuiBlockView::updateElements(bool instant)
{
    ...
    auto bubblegraph = std::make_shared<BubbleGraph>();
    auto& bubbles = bubblegraph->bubbles;
    ...
    for (size_t i = 1; i < block.vtx.size(); ++i) {
        ...
        bubblegraph->min_x = std::min(bubblegraph->min_x, proposed.pos.x() - proposed.radius);
        bubblegraph->max_x = std::max(bubblegraph->max_x, proposed.pos.x() + proposed.radius);
        bubblegraph->min_y = std::min(bubblegraph->min_y, proposed.pos.y() - proposed.radius);
    ...
    QMetaObject::invokeMethod(this, "updateSceneInit", Qt::QueuedConnection, Q_ARG(std::shared_ptr<BubbleGraph>, bubblegraph), Q_ARG(bool, instant));
}

void GuiBlockView::updateSceneInit(const std::shared_ptr<BubbleGraph>& bubblegraph, const bool instant)
{
    LOCK(m_mutex);
    for (auto& bubble : bubblegraph->bubbles) {
        auto& el = *bubble.el;
        if (!el.gi) {
            ...
            gi->setPos(bubble.pos.x() - bubble.radius, instant ? (bubble.pos.y() - bubble.radius) : offscreen);
        }
    }
    for (auto it = m_elements.begin(); it != m_elements.end(); ) {
        ...
        if (target_loc.y() == offscreen) {
            ...
            if (gi->y() == offscreen) {
                gi->setY(bubblegraph->min_y - gi->boundingRect().height());
            }
        }
        if (delete_el) {
            m_scene->removeItem(gi);
    ...
    m_scene->setSceneRect(bubblegraph->min_x, bubblegraph->min_y, bubblegraph->max_x - bubblegraph->min_x, -bubblegraph->min_y);
    if (!instant) {
        m_frame_div = 4;
        updateScene();
        m_timer.start(100);
